...
- **Deployment & env:** Environments span preview/staging/production with Paddle and Supabase keys plus NextAuth secret. Branching is `main`→prod, `develop`→staging, feature branches→previews; Vercel rollbacks are one-click and alerts cover usage, errors, and webhook failures.【F:docs/13-deployment-env.md†L3-L18】
