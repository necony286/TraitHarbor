...
export const buildReportTraitData = (
...
const resolveReportFileKey = async (order: OrderDetail, reportPath: string) => {
  ...
  if (existingAsset?.path) {
    await updateOrderReportFileKey({
      orderId: order.id,
      reportFileKey: existingAsset.path,
    });
...
export const getOrCreateReportDownloadUrl = async ({
  ...
  const pathsToTry = [
    ...new Set([reportPath, reportFileKey, legacyReportPath]),
  ].filter(Boolean);
  ...
  if (!order.response_id) {
    throw new ReportGenerationError(
      'RESULT_NOT_FOUND',
      'Result not attached to order.'
    );
  }

  const { data: traits, error: resultError } = await getScoresByResultId(
    order.response_id
  );
  ...
  const { traitPercentages, traitRankOrder, highestTrait, lowestTrait } =
    buildReportTraitData(parsedResult.data.traits);
  const { data: facetScores, error: facetError } =
    await getFacetScoresByResultId(order.response_id);
  ...
  if (!signedUrl) {
    throw new ReportGenerationError(
      'SIGNED_URL_MISSING',
      'Unable to create signed report URL.'
    );
  }
