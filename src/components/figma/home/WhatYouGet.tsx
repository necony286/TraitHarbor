...
const EXAMPLE_INSIGHTS = [
  ...
  },
  {
    ...
    description: 'High conscientiousness indicates you thrive with clear plans and systematic processes.'
  },
  {
    tag: 'Relationships',
    icon: Heart,
    headline: 'Balanced emotional expression',
    description: 'Moderate agreeableness means you balance empathy with healthy boundaries.'
  }
];
      ...
      <Container>
        ...
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-12">
          <div className="bg-card border border-border rounded-2xl p-6 lg:p-8 shadow-md">
            <h3 className="font-medium text-foreground mb-6">Your Big Five Profile</h3>
              ...
              {DETAILED_TRAITS.map((trait) => (
                <div key={trait.name} className="space-y-2">
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="text-muted-foreground font-medium">{trait.score}</span>
                  </div>
                  <div className="h-3 bg-muted rounded-full overflow-hidden">
                    <div className="h-full rounded-full bg-primary transition-all duration-500" style={{ width: `${trait.score}%` }} />
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="pt-4 border-t border-border">
              <p className="text-sm text-muted-foreground">Includes facet-level insights for each trait</p>
            </div>
          </div>
                ...
                >
                  <div className="flex items-start gap-4">
                    ...
                    <div className="space-y-1">
                      <div className="inline-block px-2 py-0.5 rounded bg-primary/10 text-xs font-medium text-primary mb-2">
                        {insight.tag}
                      </div>
